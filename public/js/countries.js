...
let createCountryList = () => {
    ...
    country_list.forEach((country, index) => {
        if( index % Math.ceil( numOfCountries / numOfLists ) == 0) {
            ul_list_id = `list${i}`;
            country_list_element.insertAdjacentHTML('beforeend', `<ul id="${ul_list_id}"></ul>`);
            ++i;
        }
        document.getElementById(ul_list_id).insertAdjacentHTML('beforeend', `
         <li id="${country.name}">${country.name}</li>
        `);
        document.getElementById(country.name).addEventListener('click', async() => {
